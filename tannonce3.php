...
if (
    ...
    if (isset($_SESSION['email']) && ($_SESSION['mot'])) {
        ...
        if ($tableau1 == false) {
            header("location:annonce3.php?errur=email ou mot de passe incorecte");
        } elseif (password_verify($mot, $tableau1['mot_passe'])) {

            if ($_POST['type'] == "Choisi le Type de Matérial" || $_POST['ville'] == "Choisi la ville") {
                header("location:annonce3.php?errur=Vous avez pas bien utiliser la liste des choix");
                ...
                $req3 = $pdo->prepare("insert into materiaux (type,titre,etat,ville,adresse,prix,description,image1,image2,
            image3,image4,email,id_cat,prix1,prix2,prix3) values(:type,:titre,:etat,:ville,:adresse,
            :prix,:des,:image1,:image2,:image3,:image4,:email,:categorie,:prix1,:prix2,:prix3)");
                ...
                if ($isok) {
                    ...
                    header("location:annonce3.php?errur=Bien Enregistrer");
                } else {
                    // si enregistrement est pas valider
                    header("location:annonce3.php?errur=Probléme D'enregistrement");
                }
            } else {
                header("location:annonce3.php?errur=taille du fichier dépasse la limite autoriser");
            }
        } else {
            header("location:annonce3.php?errur=Données de connexion sont incorrecte");
        }
    }
    ...
    //2eme partie
        ...
        if ($tableau1 == false) {
            ...
            if ($_POST['type'] == "Choisi le Type de Matérial" || $_POST['ville'] == "Choisi la ville") {
                ...
                $req3 = $pdo->prepare("insert into materiaux (type,titre,etat,ville,adresse,prix,description,image1,image2,
            image3,image4,email,mot_passe,id_cat,prix1,prix2,prix3) values(:type,:titre,:etat,:ville,:adresse,
            :prix,:des,:image1,:image2,:image3,:image4,:email,:mot,:categorie,:prix1,:prix2,:prix3)");
                $req3->bindvalue(':type', $type);
                $req3->bindvalue(':titre', $titre);
                $req3->bindvalue(':etat', $etat);
                $req3->bindvalue(':ville', $ville);
